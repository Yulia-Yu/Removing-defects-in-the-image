...
class PyPhotoEditor:
    ...
    def drow_menu(self):
        ...
        filter_menu.add_command(label="Инородные предметы",
                                command=self.apply_foreign_objects_to_current_image)  # Устраняет инородные предметы на поверхности эмульсии
        filter_menu.add_command(label="Восстановление изображения",
                                command=self.apply_image_restoration_to_current_image)  # Устраняет заломы на фото
        ...
        menu_bar.add_command(label="Help", command=self.open_help_file)
    ...
    def open_help_file(self):
        help_file_path = "path_to_your_help_file.txt"  # Specify the path to your text file
        try:
            with open(help_file_path, "r", encoding="utf-8") as file:
                file_contents = file.read()
                self.show_help_contents(file_contents)
        except FileNotFoundError:
            messagebox.showerror("Error", "Help file not found")

    def show_help_contents(self, contents):
        help_window = Toplevel()
        help_window.title("Help")
        help_text = Text(help_window, wrap=WORD)
        help_text.insert(END, contents)
        help_text.config(state=DISABLED)  # Disable editing
    ...
    def apply_median_filter_to_current_image(self):  # Шум #Изменено # Шум (старый алгоритм)
        image = self.get_current_working_data()
        if not image:
            return
    ...
    def apply_foreign_objects_to_current_image(self):  # Инородные предметы
        image = self.get_current_working_data()
        if not image:
            return

        image.apply_foreign_objects()
        image.unsaved = True
        self.update_image_inside_app(image)

    def apply_image_restoration_to_current_image(self):  # Восстановление изображения
        image = self.get_current_working_data()
        if not image:
            return

        image.apply_image_restoration()
        image.unsaved = True
        self.update_image_inside_app(image)
