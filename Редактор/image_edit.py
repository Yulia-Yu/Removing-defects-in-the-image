...
class ImageEdit:
    ...
    def rotate(self, degrees):
        self.image  = self.image.rotate(degrees, expand=True)  # поворот изображения с сохранением размера
    ...
    def apply_median_filter(self):  # Шум (старый алгоритм)
        img = np.array(self.image)

        image_numpy = cv2.fastNlMeansDenoisingColored(img, None, 7, 10, 7, 21)
    ...
    def apply_perspective(self): #Горизонт
        ...
        # Примените бинарный порог для выделения белых областей
        _, thresh = cv2.threshold(gray, 253, 255, cv2.THRESH_BINARY)

        # Инвертируйте изображение для нахождения областей, отличных от белого
        ...
        cropped_image = rotate[y:y + h, x:x + w]

        self.image = Image.fromarray(np.uint8(cropped_image))
    ...
    def chromatic_abberation(self):
        ...
        # Устанавливаем ограничение для нахождения границы в G канале # раньше было 30
        threshold = 7
    ...
    def apply_foreign_objects(self):
        img = np.array(self.image)
        # Преобразование изображения в оттенки серого
        gray_image = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)

        # Применение пороговой обработки для выделения пылинок
        _, thresh = cv2.threshold(gray_image, 170, 250, cv2.THRESH_BINARY_INV)
        ...
        min_area = 25
        # Заполнение областей внутри контуров цветом
        for contour in contours:
            Area = cv2.contourArea(contour)
            if Area < min_area:
                cv2.drawContours(cleaned_image, [contour], 0, (255, 255, 255), thickness=-1)
        ...
        # Отрисовка контуров на изображении
        cv2.drawContours(binary_image, contours, -1, (255, 255, 255), 5)

        cleaned_image = cv2.inpaint(cleaned_image, binary_image, 9, cv2.INPAINT_TELEA)

        # Для темных точек
        gray_image = cv2.cvtColor(cleaned_image, cv2.COLOR_BGR2GRAY)
        _, thresh = cv2.threshold(gray_image, 55, 56, cv2.THRESH_BINARY_INV)  # 150 180
        ...
        # Отрисовка контуров на изображении
        cv2.drawContours(binary_image, contours, -1, (255, 255, 255),
                         3)  # Отрисовка всех контуров зеленым цветом толщиной 3 пикселя

        cleaned_image = cv2.inpaint(cleaned_image, binary_image, 2, cv2.INPAINT_TELEA)

        self.image = Image.fromarray(np.uint8(cleaned_image))


    def apply_image_restoration(self):
        img = np.array(self.image)

        # Преобразуем изображение в биективное
        gray_image = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        ...
        cv2.drawContours(contour_image, contours, -1, (255, 255, 255), 17)  # Отрисовка всех контуров зеленым цветом толщиной 3 пикселя

        # Закрашивание обнаруженных дефектов
        cleaned_image = cv2.inpaint(img, contour_image, 1, cv2.INPAINT_TELEA)
        self.image = Image.fromarray(np.uint8(cleaned_image))
